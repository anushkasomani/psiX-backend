...
class ContractManager {
     ...
     */
    async processBatch(assetIds, oldRoots, newRoots, netDeltas, marginDeltas) {
        ...
        try {
            ...
            const tx = await this.perpEngineZK.processBatch(assetIds, oldRoots, newRoots, netDeltas, marginDeltas, {
     ...
     */
    async initializeAsset(assetId, initialRoot) {
        if (!this.perpEngineZK) {
            throw new Error('PerpEngineZK contract not initialized');
        }
        try {
    ...
    getFallbackPrice(assetId) {
        ...
        return fallbackPrices[assetId] || 1000n * 10n ** 18n;
    }
    // ====================================================================
    ...
    async getFeeConfig() {
        ...
        try {
            const [openFeeBps, closeFeeBps, borrowingRateAnnualBps, minCollateralRatioBps, maxUtilizationBps] = await Promise.all([
