...
const close_executor_1 = require("./close-executor");
// ====================================================================
...
app.post('/setup/crypto', async (req, res) => {
    try {
        ...
        const publicKey = await crypto_1.cryptoManager.initialize(forceRegenerate);
        res.json({
            success: true,
...
app.get('/crypto/public-key', (req, res) => {
    try {
        const publicKey = crypto_1.cryptoManager.getPublicKey();
        res.json({
            success: true,
            publicKey,
            timestamp: new Date().toISOString()
...
app.post('/balance/add', (req, res) => {
    try {
        ...
        const newBalance = database_1.database.getUserBalance(user);
        res.json({
            success: true,
...
app.get('/balance/:user', (req, res) => {
    try {
        ...
        const balance = database_1.database.getUserBalance(user);
        res.json({
            success: true,
            user,
            balance: {
...
app.get('/balance/all', (req, res) => {
    try {
        const allBalances = database_1.database.getAllBalances();
        res.json({
            success: true,
...
app.post('/trade/submit', async (req, res) => {
    try {
        ...
        const encryptedData = { enc, ct };
        const processedTrade = await executor_1.executor.processEncryptedTrade(encryptedData);
        res.json({
            success: true,
...
app.post('/trade/create-sample', async (req, res) => {
    try {
        ...
        const encrypted = await crypto_1.cryptoManager.createSampleEncryptedTrade(overrides);
        res.json({
            success: true,
...
app.post('/trade/create-test', async (req, res) => {
    try {
        ...
        const encrypted = await crypto_1.cryptoManager.encryptJSON(testPayload);
        res.json({
            success: true,
...
app.post('/trade/create-close', async (req, res) => {
    try {
        ...
        const encrypted = await crypto_1.cryptoManager.createSampleClosePosition(closeRequest);
        res.json({
            success: true,
...
app.post('/trade/close', async (req, res) => {
    try {
        ...
        const encryptedData = { enc, ct };
        const processedClose = await close_executor_1.closeExecutor.processEncryptedClose(encryptedData);
        res.json({
            success: true,
...
app.get('/batch/pending', (req, res) => {
    try {
        const pendingTrades = executor_1.executor.getPendingTrades();
        res.json({
            success: true,
...
app.post('/batch/process', async (req, res) => {
    try {
        ...
        const result = await executor_1.executor.forceBatchProcessing();
        if (!result) {
            return res.json({
                success: true,
...
app.get('/close/pending', (req, res) => {
    try {
        const pendingCloses = close_executor_1.closeExecutor.getPendingCloses();
        res.json({
            success: true,
...
app.post('/close/process', async (req, res) => {
    try {
        ...
        const result = await close_executor_1.closeExecutor.forceCloseBatchProcessing();
        if (!result) {
            return res.json({
                success: true,
...
app.get('/position/:user', (req, res) => {
    try {
        const { user } = req.params;
        const positions = database_1.database.getTraderPositions(user);
        res.json({
            success: true,
            user,
...
app.get('/merkle/proof/:trader/:assetId', (req, res) => {
    try {
        ...
        const proof = merkle_1.merkleTree.generateProof(trader, parseInt(assetId));
        if (!proof) {
...
app.get('/merkle/stats', (req, res) => {
    try {
        const stats = merkle_1.merkleTree.getStats();
        res.json({
            success: true,
            merkleTree: stats,
            timestamp: new Date().toISOString()
...
app.get('/pnl/:user', async (req, res) => {
    try {
        ...
        const pnlData = await close_executor_1.closeExecutor.calculateCurrentPnL(user, assetId ? parseInt(assetId) : undefined);
        res.json({
            success: true,
...
app.post('/fees/calculate', (req, res) => {
    try {
        ...
        const fees = fees_1.feeCalculator.calculateNewPositionFees(BigInt(positionSize), BigInt(margin), isLong);
        res.json({
            success: true,
            fees: {
...
app.get('/test/verify', (req, res) => {
    try {
        const merkleIntegrity = merkle_1.merkleTree.verifyIntegrity();
        res.json({
            success: true,
            integrity: {
...
async function startServer() {
    try {
        ...
        // Initialize crypto system
        await crypto_1.cryptoManager.initialize();
        // Check contract connectivity
        const connected = await contracts_1.contractManager.checkConnection();
        if (!connected) {
