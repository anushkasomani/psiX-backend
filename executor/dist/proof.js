...
const groth16_1 = require("@zk-kit/groth16");
...
async function generateAndVerifyProof(index) {
    const fieldModulus = BigInt("21888242871839275222246405745257275088548364400416034343698204186575808495617");
    const merkleRoot = BigInt((0, tree_1.currentRoot)());
    const input = {
        oldRoot: merkleRoot.toString(),
        newRoot: merkleRoot.toString(),
        size: 3,
    ...
    const baseDir = (0, path_1.resolve)(__dirname, "../../circuits-synth/outputs");
    const wasmPath = (0, path_1.resolve)(baseDir, "liquidate_js/liquidate.wasm");
    const zkeyPath = (0, path_1.resolve)(baseDir, "liquidate_final.zkey");
    try {
        const { proof, publicSignals } = await (0, groth16_1.prove)(input, wasmPath, zkeyPath);
        const { a, b, c, input: inputArr } = formatProofForSolidity(proof, publicSignals);
        console.log("a is", a);
        console.log("b is", b);
        console.log("c is", c);
