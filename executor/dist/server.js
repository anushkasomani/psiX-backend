...
const cryptography_1 = require("./executor-botv1/cryptography");
...
app.post('/crypto/generate-keys', async (req, res) => {
    try {
        ...
        const keyPair = await (0, cryptography_1.generateHPKEKeyPair)();
        // Save to files
        (0, cryptography_1.saveHPKEKeyPair)(keyPair);
        res.json({
            success: true,
...
app.get('/crypto/public-key', (req, res) => {
    try {
        const publicKey = (0, cryptography_1.loadHPKEPublicKey)();
        res.json({
            success: true,
            publicKey,
            timestamp: new Date().toISOString()
...
app.post('/crypto/initialize', async (req, res) => {
    try {
        ...
        const keyPair = await (0, cryptography_1.initializeCrypto)(forceRegenerate);
        res.json({
            success: true,
...
app.get('/crypto/verify', (req, res) => {
    const isVerified = (0, cryptography_1.verifyCryptoSetup)();
    res.json({
        success: isVerified,
...
app.post('/crypto/encrypt', async (req, res) => {
    try {
        ...
        const recipientPublicKey = publicKey || (0, cryptography_1.loadHPKEPublicKey)();
        console.log('🔐 Encrypting data...');
        console.log('📄 Data to encrypt:', JSON.stringify(data, null, 2));
        const encrypted = await (0, cryptography_1.encryptJSON)(data, recipientPublicKey);
        res.json({
            success: true,
...
app.post('/crypto/decrypt', async (req, res) => {
    try {
        ...
        const decrypted = await (0, cryptography_1.decryptJSON)(encryptedData);
        res.json({
            success: true,
...
app.post('/trade/submit', async (req, res) => {
    try {
        ...
        const encryptedData = { enc, ct };
        const decryptedTrade = await (0, cryptography_1.decryptJSON)(encryptedData);
...
app.post('/trade/encrypt-sample', async (req, res) => {
    try {
        ...
        const publicKey = (0, cryptography_1.loadHPKEPublicKey)();
        const encrypted = await (0, cryptography_1.encryptJSON)(tradeData, publicKey);
        res.json({
            success: true,
...
async function startServer() {
    try {
        ...
        await (0, cryptography_1.initializeCrypto)();
        app.listen(PORT, () => {
