...
class IMT {
    ...
    constructor(hash, depth, zeroValue, arity = 2, leaves = []) {
        ...
        errorHandlers.requireNumber(depth, "depth");
        errorHandlers.requireTypes(zeroValue, "zeroValue", ["number", "string", "bigint"]);
        errorHandlers.requireNumber(arity, "arity");
        errorHandlers.requireObject(leaves, "leaves");
     ...
     */
    indexOf(leaf) {
        errorHandlers.requireTypes(leaf, "leaf", ["number", "string", "bigint"]);
    ...
    static verifyProof(proof, hash) {
        ...
        errorHandlers.requireTypes(proof.root, "proof.root", ["number", "string", "bigint"]);
        errorHandlers.requireTypes(proof.leaf, "proof.leaf", ["number", "string", "bigint"]);
        errorHandlers.requireArray(proof.siblings, "proof.siblings");
        errorHandlers.requireArray(proof.pathIndices, "proof.pathIndices");
        let node = proof.leaf;
